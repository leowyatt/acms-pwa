 ...
 */
export default (apiUrl: string, httpClient = fetchUtils.fetchJson, uploadFields = []) => {
  /**
   ...
   */
  return (type: string, resource: string, params: object) => {
    ...
    if (type === GET_MANY_REFERENCE || type === GET_LIST) {
      return Promise.all([httpClient(url, options)]).then((promises) => {
        const response = {
          ...promises[0],
